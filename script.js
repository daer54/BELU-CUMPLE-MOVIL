document.addEventListener('DOMContentLoaded', () => {
    ...
    if (introLoader && passwordForm && passwordInput && errorMessage) {
        passwordForm.addEventListener('submit', (e) => {
            ...
            if (password.toLowerCase() === 'maria') {
                ...
                // Helper to attempt play music (safeguard for strict mobile autoplay rules)
                const tryPlayMusic = () => {
                    if (bgMusic && bgMusic.paused) {
                        ...
                        if (playPromise !== undefined) {
                            playPromise.then(() => {
                                ...
                                // Clean up temporary interaction listeners
                                document.removeEventListener('click', tryPlayMusic);
                                document.removeEventListener('touchstart', tryPlayMusic);
                            }).catch(err => {
                                console.log("Autoplay blocked by browser. Music will play on next tap.", err);
                            });
                        } else {
                            if (audioToggle) audioToggle.classList.add('playing');
                            if (audioText) audioText.textContent = 'Pausar';
                ...
                setTimeout(() => {
                    tryPlayMusic();
                    // If still paused (blocked by browser), bind temporary screen-tap fallback
                    if (bgMusic && bgMusic.paused) {
    ...
    if (envelope) {
        ...
        // Tap-to-play fallback for mobile video cards (safeguard against battery saver blocking autoplay)
        ...
        videoCards.forEach(card => {
            card.addEventListener('click', (e) => {
                ...
                if (video) {
                    if (video.paused) {
                        video.play().catch(err => console.log("Video manual play failed", err));
                    } else {
                        video.pause();
                    }
                }
